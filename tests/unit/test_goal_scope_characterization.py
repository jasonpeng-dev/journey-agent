...
def test_current_final_verification_and_objective_evaluator_are_not_equivalent() -> None:
    state = _state(
        valley_security="SAFE",
    ...
    assert "starfire_outpost_status" not in expected
...
def test_ambiguous_and_unsupported_goals_currently_have_no_resolution_lifecycle(
    ...
    _conversation, task = _task_context(session, goal)
...
def test_mock_planner_currently_ignores_goal_text() -> None:
    ...
    assert first.tool_calls[0].arguments == initial_strategic_starfire_plan(task_id)


def test_replan_context_currently_has_goal_text_but_no_frozen_scope(
    session: Session,
) -> None:
...
def _task_context(session: Session, goal: str) -> tuple[ConversationSession, AgentTask]:
    ...
    session.flush()
    task = TaskService(session).create_task(conversation, goal, "starfire_command")
