...
export function GameCards({ games, archived }: { games: GameSummary[] | undefined; archived: boolean }) {
  ...
  return <>
    {(archive.error || remove.error || fork.error) && <p className="error-text">{errorText(archive.error ?? remove.error ?? fork.error)}</p>}
    <div className="card-grid">{games.map((game) => <article className="scenario-card game-card" key={game.id}>
      ...
      <div className="game-card-actions"><Link className="secondary-button" to={`/games/${game.id}`}>{archived ? "查看记录" : "继续游戏"}</Link>{archived && <button disabled={fork.isPending || remove.isPending} onClick={() => fork.fork(game.id)}>以归档状态新开一局</button>}{!archived && <button disabled={archive.isPending || remove.isPending || Boolean(game.active_task_id)} title={game.active_task_id ? "当前有活动任务，完成或放弃后才能归档" : undefined} onClick={() => archive.mutate({ id: game.id, revision: game.runtime_revision })}>结束并归档</button>}<button className="danger-button" disabled={archive.isPending || remove.isPending || fork.isPending} onClick={() => permanentlyDelete(game.id)}>永久删除</button></div>
    </article>)}</div>
  </>;
}
