...
export function GamePage() {
  ...
  return <main className="page play-page">
    ...
    {task && <section className="detail-card"><p className="eyebrow">Current Goal</p><h2>{task.goal}</h2><p>{task.objective_names.join(" · ")} · {task.status}</p>
      ...
      {play.data.pending_approval_id && <div className="approval-box"><p>This action needs your decision.</p><button onClick={() => decision.mutate({ approve: true, decisionId: play.data.pending_approval_id!, taskVersion: task.version })}>Approve</button><button className="danger" onClick={() => decision.mutate({ approve: false, decisionId: play.data.pending_approval_id!, taskVersion: task.version })}>Reject &amp; Replan</button></div>}
    ...
    <section className="developer-toggle"><button className="small" onClick={() => setDeveloperOpen((value) => !value)}>Developer View</button>{developerOpen && <div className="developer-panel"><label>Developer credential<input type="password" value={developerToken} onChange={(event) => setDeveloperToken(event.target.value)} /></label>{developer.error && <p className="error">Developer access denied.</p>}{developer.data && <><h2>Internal Runtime Snapshot</h2><pre>{JSON.stringify(developer.data, null, 2)}</pre></>}</div>}</section>
  </main>;
}
