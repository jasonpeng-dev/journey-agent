...
export function NewScenarioPage() {
  ...
  const [exampleKey, setExampleKey] = useState("medical_emergency");
  const create = useMutation({
    mutationFn: () => api.createScenario({ mode, key, name, ...(mode === "EXAMPLE" ? { example_key: exampleKey } : {}) }),
  ...
  return <main className="page"><p className="eyebrow">场景创作</p><h1>新建场景</h1><div className="form-card">
    ...
    {mode === "EXAMPLE" && <label>示例<select value={exampleKey} onChange={(event) => setExampleKey(event.target.value)}>{examples.data?.map((item) => <option key={item.key} value={item.key}>{item.name} · {uiLabel(item.maturity)}</option>)}</select></label>}
    ...
    <button className="primary-button" disabled={!key || !name || create.isPending} onClick={() => create.mutate()}>{create.isPending ? "正在创建……" : "创建当前草稿"}</button>
