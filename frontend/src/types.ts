...
export type PublicPlanHistoryStep = { id: string; sequence: number; action_name: string; assigned_actor_name: string; status: "PLANNED" | "CURRENT" | "COMPLETED" | "FAILED" | "CANCELLED"; result_summary: string | null };
export type PublicPlanHistory = { id: string; ordinal: number; status: "EXECUTING" | "ADJUSTED" | "COMPLETED" | "BLOCKED"; completed_steps: number; total_steps: number; failed_step_name: string | null; steps: PublicPlanHistoryStep[] };
...
export type TimelineEventKind = "TASK_STARTED" | "ACTION_BRIEFING" | "ACTION_RESULT" | "PLAN_UPDATED" | "APPROVAL_REQUIRED" | "APPROVAL_APPROVED" | "APPROVAL_REJECTED" | "TASK_COMPLETED" | "TASK_BLOCKED" | "TASK_ABORTED";
export type KnowledgeChange = { kind: "NODE_REVEALED" | "FACT_REVEALED"; key: string; name: string; value: string | number | boolean | null };
export type PublicTimelineEvent = { id: string; kind: TimelineEventKind; title: string; detail: string | null; actor_name: string | null; result_summary: string | null; success: boolean | null; knowledge_changes: KnowledgeChange[]; occurred_at: string | null };
export type ExecutionPhase = "AWAITING_ACTION_ACK" | "AWAITING_DEBRIEF_ACK" | "APPROVAL_REQUIRED" | "COMPLETED" | "BLOCKED" | "ABORTED";
export type ActionBriefing = { step_id: string; action_name: string; actor_name: string; target_name: string; purpose: string };
export type ActionDebrief = { step_id: string; action_name: string; success: boolean; result_summary: string; knowledge_changes: KnowledgeChange[]; plan_adjusted: boolean; plan_adjustment_summary: string | null };
...
export type PlayerGameState = { game: GameSummary; visible_nodes: Array<{ key: string; name: string; accessible: boolean }>; known_facts: Array<{ node_key: string; fact_key: string; name: string; value: string | number | boolean }>; resources: Array<{ key: string; name: string; value: number; reserved_value: number }>; actors: Array<{ key: string; name: string; role_name: string; current_node_name: string }>; current_task: PublicTask | null; pending_approval_id: string | null };
