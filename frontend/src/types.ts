...
export type PublicPlanningDraftStep = { step_id: string; purpose: string; action_key: string | null; actor_key: string | null; target_key: string | null; parameters: Record<string, unknown> };
export type PublicPlanningViolation = { category: string; dimension: string | null; summary: string; step_id: string | null; actor_key: string | null; action_key: string | null; target_key: string | null };
export type PublicPlanningAttempt = { id: string; attempt_index: number; call_type: "INITIAL_PLAN" | "REPLAN" | "REPAIR"; status: "RUNNING" | "REJECTED" | "ACCEPTED" | "ERROR"; stop_reason: string | null; proposal_steps: PublicPlanningDraftStep[]; validator_violations: PublicPlanningViolation[]; started_at: string | null; finished_at: string | null; latency_ms: number | null };
export type PublicPlanningCycle = { id: string; base_call_type: "INITIAL_PLAN" | "REPLAN"; status: string; current_attempt: number; attempts: PublicPlanningAttempt[] };
...
export type PublicTask = { id: string; version: number; goal: string; status: string; execution_phase: ExecutionPhase; pacing_version: number; objective_names: string[]; roadmap: { stages: MissionRoadmapStage[] }; plan: PublicPlan | null; plan_history: PublicPlanHistory[]; planning_cycle?: PublicPlanningCycle | null; timeline: PublicTimelineEvent[]; briefing: ActionBriefing | null; debrief: ActionDebrief | null; explanation: string | null };
